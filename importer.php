...
$result = $mysqli->query("SELECT DISTINCT CodeLevel3, DescriptionLevel1, DescriptionLevel2, DescriptionLevel3 FROM categories ORDER BY CodeLevel3 ASC");
$counter = 1;
while ($row = $result->fetch_assoc()) {
	$result0 = $mysqli->query("SELECT * FROM categorias WHERE nombre ='".addslashes($row['DescriptionLevel2'])."'");
	if ($result0) {
		$papaloi = $result0->fetch_assoc();
		$mysqli->query("INSERT INTO categorias (nombre,parent_id,codigo) VALUES ('".$row['DescriptionLevel3']."',".$papaloi['id'].",'".$row['CodeLevel3']."')");
	}
}
/*
$mysqli = new mysqli("localhost", "root", "", "tienda");
